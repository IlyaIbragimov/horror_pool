...
export function WatchlistItemCard({ watchlistItem }: Props) {
  const posterUrl = watchlistItem.movieDTO.posterPath ? `${TMDB_IMG_BASE}${watchlistItem.movieDTO.posterPath}` : null;

  return (
    <div className={styles.watchlistItem_card}>

      <Link key={watchlistItem.movieDTO.movieId} to={`/movies/${watchlistItem.movieDTO.movieId}`} className={styles.watchlistItem_card_posterWrap}>
        {posterUrl ? (
          <img className={styles.watchlistItem_card_poster} src={posterUrl} alt={watchlistItem.movieDTO.title} />
        ) : (
      ...
      <div className={styles.watchlistItem_card_content}>
        <h3 className={styles.watchlistItem_card_title}>{watchlistItem.movieDTO.title}</h3>
        ...
        <div className={styles.watchlistItem_card_isWatched}>
          <label className={styles.watchlistItem_checkboxLabel}>
            <input type="checkbox" checked={Boolean(watchlistItem.watched)} readOnly />
            <span>Watched</span>
          </label>
        </div>
      </div>
